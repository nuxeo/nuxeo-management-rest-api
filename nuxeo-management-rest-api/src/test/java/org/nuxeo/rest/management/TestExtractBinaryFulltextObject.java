...
public class TestExtractBinaryFulltextObject extends ManagementBaseTest {
    ...
    public void testRunExtractFulltext() throws IOException {
        ...
        Map<String, String> ft = doc.getBinaryFulltext();
        ...
        assertTrue(ft.toString(), ft.get(BINARY_FULLTEXT_MAIN_KEY).isEmpty());
    ...
    protected void runExtractFulltext() throws IOException {
        ...
        try (CloseableClientResponse response = httpClientRule.post("site/management/fulltext/extract", formData)) {
            assertEquals(SC_OK, response.getStatus());
            JsonNode node = mapper.readTree(response.getEntityInputStream());
            assertEquals(BulkStatus.State.SCHEDULED.name(), node.get("state").asText());
            commandId = node.get(STATUS_COMMAND_ID).asText();
        }
        txFeature.nextTransaction();
        try (CloseableClientResponse response = httpClientRule.get("site/management/bulk/" + commandId)) {
            JsonNode node = mapper.readTree(response.getEntityInputStream());
            assertEquals(SC_OK, response.getStatus());
            assertEquals(BulkStatus.State.COMPLETED.name(), node.get("state").asText());
            assertEquals(node.toString(), false, node.get(STATUS_HAS_ERROR).asBoolean());
        }
    }
